...
import { demoInviteDraft, demoPendingInvites } from '../demo/fixtures'
...
export function HouseholdPage() {
  ...
  const isJourney = pathname === '/__journey__/household'
  const { user, signOut } = useAuth()
  const { household, members, updateHouseholdName, refresh } = useHousehold()
  const { pref, setPref } = useTheme()
  const [name, setName] = useState(household?.name ?? '')
  const [message, setMessage] = useState<string | null>(null)
  const [inviteEmail, setInviteEmail] = useState(isJourney ? demoInviteDraft.email : '')
  const [inviteMsg, setInviteMsg] = useState<string | null>(null)
  const [inviting, setInviting] = useState(false)
  const [invites, setInvites] = useState<HouseholdInvite[]>(isJourney ? demoPendingInvites : [])
  const [memberEmails, setMemberEmails] = useState<Record<string, string>>({})
  const [isAdmin, setIsAdmin] = useState(false)
  const [acceptMsg, setAcceptMsg] = useState<string | null>(null)

  const loadInvites = useCallback(async () => {
    if (!household || isJourney) return
    ...
    <div className="space-y-5">
      ...
      <section className="card space-y-3 p-4">
        ...
        <form onSubmit={handleInvite} className="flex gap-2">
          ...
          />
          <button type="submit" disabled={inviting || isJourney} className="btn-primary px-4 text-sm">
            {inviting ? '…' : 'Invite'}
          </button>
        </form>
